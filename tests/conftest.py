...
def fill_db():
    # admin role
    admin = models.User(public_id=ADMIN_ID, name='admin',
                        password=generate_password_hash('admin'), role=Role.ADMIN)
    db.session.add(admin)

    # user1 role
    user1 = models.User(public_id=USER1_ID, name='user1',
                        password=generate_password_hash('user1'), role=Role.USER1)
    db.session.add(user1)

    # user2 role
    user2 = models.User(public_id=USER2_ID, name='user2',
                        password=generate_password_hash('user2'), role=Role.USER2)
    db.session.add(user2)

    # source
    source = models.Source(public_id=SOURCE_ID, owner_id=USER2_ID,
                           name='device1', password=generate_password_hash('source'), role=Role.SOURCE, owner=user2)
    db.session.add(source)

    # ImageClassifier
    with open('./tests/ImageClassifier.h5', 'rb') as f:
        model_h5 = models.Model(public_id=CLASSIFIER_ID, name=f.name, binary=f.read(
        ), config={'shape': (32, 32), 'expand_dims': True}, type='.hdf5')
        db.session.add(model_h5)

    # MobileNetClassifier Built-in
    model_mobile = models.Model(public_id=MOBILENET_ID, name='MobileNet',
                                config={'shape': (224, 224), 'expand_dims': True})
    db.session.add(model_mobile)

    # request from user1
    request_user1 = models.Request(public_id=USER1_REQ_ID, user_id=USER1_ID,
                                   model_id=CLASSIFIER_ID, input=BYTE_IMAGE, output='[test_output]',
                                   status=RequestStatus.FINISHED,
                                   user=user1, model=model_h5)
    db.session.add(request_user1)

    # request from source
    request_source = models.Request(public_id=SOURCE_REQ_ID, user_id=USER2_ID, source_id=SOURCE_ID,
                                    model_id=CLASSIFIER_ID, input=BYTE_IMAGE, output='[test_output]',
                                    status=RequestStatus.FINISHED,
                                    user=user2, source=source, model=model_mobile)
...
class TestClient(object):
    ...
    def login(self, name, password, source=False, use_cookie=False):
        response = self._client.post(
            'auth/login', json={"name": name, "password": password, "source": source, "use_cookie": use_cookie})
    ...
    def get(self, uri):
        response = self._client.get(
            uri, headers={'x-access-token': self._token})
        return response
