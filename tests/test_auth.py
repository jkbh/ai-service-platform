...
from conftest import USER1_ID


def create_token(key: str, public_id: str, expired: bool = False):
    if expired:
        expire_date = datetime.datetime.utcnow() - datetime.timedelta(1)
    else:
        expire_date = datetime.datetime.utcnow() + datetime.timedelta(1)
    return jwt.encode({'public_id': public_id, 'exp': expire_date}, key, algorithm='HS256')
...
@pytest.mark.parametrize(('public_id', 'expired'), [(USER1_ID, True), ('wrong_id', False)])
...
@pytest.mark.parametrize(['name', 'password', 'code'],
                         [('admin', 'admin', 200), ('user1', 'user1', 200), ('user1', 'wrong_password', 400)])
...
@pytest.mark.parametrize(('name', 'password'), [('user1', 'wrong_password'), ('wrong_user', 'user1')])
